...
/**
 * @brief Generates a random vector of complex<scalar> values, using a
...
static inline void fill_random_with_range(
  ...
  for (complex_std<scalar_t> &e : vec) {
    e = complex_std<scalar_t>{random_scalar(rangeMin, rangeMax),
                              random_scalar(rangeMin, rangeMax)};
  }
}

/**
 * @brief Generates a random vector of complex<scalar> values, using a uniform
 * distribution.
 */
template <typename scalar_t>
