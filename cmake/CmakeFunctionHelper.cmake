...
function(cpp_type output data)
  ...
  elseif(${data} STREQUAL "complex<double>")
    ...
    return()
  elseif(${data} STREQUAL "bfloat16")
    set(${output} "cl::sycl::ext::oneapi::bfloat16" PARENT_SCOPE)
    return()
  endif()
...
function(generate_blas_objects blas_level func)
  ...
  if(BLAS_ENABLE_HALF)
    ...
    if("${func}" IN_LIST BFLOAT16_DATA_OPS)
        list(APPEND data_list_c "bfloat16")
    endif()
  endif()
  foreach(data_in ${data_list_c})
    ...
    if(((data_in STREQUAL "half") OR (data_in STREQUAL "bfloat16")) AND (${func} STREQUAL "gemm"))
...
function(add_gemm_configuration
  ...
  # Same for bfloat16-float and bfloat16-bfloat16
  set(data_list_out ${data})
  set(data_list_in ${data})
  if((data STREQUAL "half"))
    list(APPEND data_list_out "float")
    list(APPEND data_list_in "bfloat16")
    list(APPEND data_list_out "bfloat16")
  endif()
  foreach(data_in ${data_list_in})
    cpp_type(cpp_data ${data_in})
    foreach(data_out ${data_list_out})
      # Mixed precision not valid with all combination unless out=float
      if((NOT data_in STREQUAL data_out) AND (NOT data_out STREQUAL "float"))
        continue()
      endif()
      cpp_type(cpp_data_out ${data_out})
      foreach(symm_a ${boolean_list})
        foreach(symm_b ${boolean_list})
          if ((${data_in} MATCHES "half") AND (symm_a OR symm_b))
            continue()
          endif()
              ...
              foreach(is_beta_zero ${boolean_list})
                foreach(index ${index_list})
                  set(file_name "${func}_${double_buffer}_${conflict_a}_"
                          ...
                          "${wg_size}_${cache_line_size}_${data_in}_${data_out}.cpp")
